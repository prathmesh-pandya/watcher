...
async function main(): Promise<void> {
  ...
  const worker = new Worker<PushJobData>(
    PUSH_QUEUE_NAME,
    (job: Job<PushJobData>) => processPushJob(job),
    {
      connection,
  ...
  worker.on("completed", (job, result) => {
    logger.info(
      { jobId: job.id, repo: job.data.repoFullName, result },
      "job completed",
    );
  });
  ...
  logger.info(
    { queue: PUSH_QUEUE_NAME, concurrency: env.WORKER_CONCURRENCY },
    "worker listening",
  );
  ...
  const shutdown = async (signal: string) => {
    ...
    await disconnectMongo().catch((err) =>
